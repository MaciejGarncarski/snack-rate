...
export function useRemoveReply({ parentCommentId }: { parentCommentId: string }) {
  return useMutation(
    orpc.comments.removeReply.mutationOptions({
      onError: (error) => {
        const message = error instanceof Error ? error.message : undefined;
        toast.error(message ?? "Nie udało się usunąć odpowiedzi");
      },
