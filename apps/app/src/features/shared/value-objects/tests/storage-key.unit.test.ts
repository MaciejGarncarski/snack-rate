...
describe("StorageKey value object", () => {
  describe("create", () => {
    it("should create a valid StorageKey with slug and image extension", () => {
      ...
      expect(value).toMatch(/^monster-energy-[a-z0-9]{5}\.[a-zA-Z0-9_-]{21}\.jpg$/u);
    });
  ...
  describe("createThumb", () => {
    it("should create a valid thumbnail StorageKey with slug and image extension", () => {
      const slug = Slug.create("Monster Energy");
      const storageKey = StorageKey.createThumb(slug, "jpg");
      const value = storageKey.getValue();

      expect(value).toMatch(/^monster-energy-[a-z0-9]{5}\.[a-zA-Z0-9_-]{21}-thumb\.jpg$/u);
    });

    it("should throw when imageExtension is empty", () => {
      const slug = Slug.create("Chips");
      expect(() => StorageKey.createThumb(slug, "")).toThrow(
        "StorageKey imageExtension cannot be empty",
      );
    });
