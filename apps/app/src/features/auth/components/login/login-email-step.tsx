...
export function LoginEmailStep({ email, onEmailChange, onCodeSent }: LoginEmailStepProps) {
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        void sendCode({ email, token: token ?? "" });
      ...
      <FieldGroup>
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-balance text-muted-foreground">
            Zaloguj się do swojego konta SnackRate
          </p>
        </div>
        <Field>
        ...
        <Field>
          <Button type="submit" isDisabled={isSending}>
            {isSending ? "Wysyłanie..." : "Wyślij kod weryfikacyjny"}
          </Button>
        </Field>
