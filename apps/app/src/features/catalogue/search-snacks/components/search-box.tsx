      ...
      <AnimatePresence mode="wait">
        {suggestionsOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute left-1/2 backdrop-blur-xl -translate-x-1/2 mt-2 w-58 rounded-lg border border-accent bg-popover/85 text-sm shadow-xl md:top-11 md:w-84"
          >
            {data?.length === 0 ? (
