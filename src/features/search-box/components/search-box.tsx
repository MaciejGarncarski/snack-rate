      ...
      <AnimatePresence>
        {suggestionsOpen && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="absolute top-full -left-10 mt-1 w-58 rounded border bg-popover p-1 text-sm shadow-lg md:-left-10 md:w-84 md:p-2"
          >
            {data?.length === 0 ? (
              <div className="p-4 text-muted-foreground">Brak wyników</div>
            ) : isLoading ? (
              <div className="p-4">Ładowanie...</div>
            ) : (
              <ul ref={suggestionListContainerRef}>
                {data?.map((item, index) => (
                  <li
                    key={item.slug}
                    className={cn(
                      `border-b px-4 py-2 last:border-b-0 hover:bg-accent md:px-6 md:py-3`,
                      index === selectedIndex ? "md:bg-accent" : "",
                    )}
                  >
                    <Link
                      to="/product/$slug"
                    ...
                    >
                      <div className="mb-1 size-10 overflow-hidden rounded-xs">
